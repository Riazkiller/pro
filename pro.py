...
try:
    rq = requests.get('https://pastebin.com/raw/NWjcXQPb').text
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            ...
            try:
                ...
                for x in ppp:yyy = x.replace('•',' • ');print(' %s[%sKachi-OK%s] %s'%(H,P,H,yyy))
        ...
        try:
            cpl = os.listdir("Kachi-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sKachi-CP%s] %s          "%(K,P,K,kontol)
